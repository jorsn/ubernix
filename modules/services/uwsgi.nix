...
let
  ...
  buildCfg = name: c:
    ...
    uwsgiCommand = "${uwsgi}/bin/uwsgi --json ${buildCfg "server" cfg.instance}/server.json";
    runWithRunDir = pkgs.writeShellScript "run-uwsgi"
      "[ -d ${cfg.runDir} ] || mkdir -p ${cfg.runDir} && ${uwsgiCommand}";
in {
  ...
  options = {
    services.uwsgi = {
      ...
      runDir = mkOption {
        type = types.path;
        default = "/run/user/$UID/uwsgi";
        description = "Where uWSGI communication sockets can live";
      };

      createRunDir = mkEnableOption "Create runDir if it does not exist.";
  ...
  config = {
    lib.uwsgi = {
      mkVassal = name: cfg: f: let args = f cfg; in {
        inherit (args) plugins;
        enable = true;
        instance = {
          type = "emperor";
          vassals.${name} = { type = "normal"; } // lib.filterAttrs (n: v: n != "plugins") args;
        };
      };
    };
    services.supervisord.services.uwsgi = mkIf cfg.enable {
      command = "${if cfg.createRunDir then runWithRunDir else uwsgiCommand}";
      settings = { stopsignal = "INT"; };
    };
  };
}
